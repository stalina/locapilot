...
import { useInventoryPhotos } from '@/shared/composables/useInventoryPhotos';
...
async function loadPhotos() {
  photos.value = await getInventoryPhotos(props.inventoryId);
...
async function handleFileSelect(event: Event) {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    const result = await addInventoryPhoto(props.inventoryId, file);
    if (result) {
      await loadPhotos();
      emit('update');
    }
...
async function handleRemove(photoId: number) {
  ...
  await removeInventoryPhoto(props.inventoryId, photoId);
...
async function handleSetPrimary(photoId: number) {
  await setPrimaryPhoto(props.inventoryId, photoId);
  await loadPhotos();
  emit('update');
}
...
<template>
  <div class="inventory-photo-gallery">
    <!-- Error message -->
