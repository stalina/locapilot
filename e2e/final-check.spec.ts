...
test.describe('Visual Comparison - Final Check', () => {
  test('all main views have consistent structure', async ({ page }) => {
    ...
    for (let i = 1; i < results.length; i++) {
      ...
      expect(current.containerWidth, `${current.name} container width`).toBe(reference.containerWidth);
      expect(current.containerMaxWidth, `${current.name} max-width`).toBe(reference.containerMaxWidth);
      expect(current.containerPadding, `${current.name} padding`).toBe(reference.containerPadding);
      expect(current.statsGridColumns, `${current.name} stats grid`).toBe(reference.statsGridColumns);
      expect(current.contentGridColumns, `${current.name} content grid`).toBe(reference.contentGridColumns);
