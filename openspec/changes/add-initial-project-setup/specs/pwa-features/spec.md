...
The application MUST être installable comme une application native sur macOS, Windows et Linux via le protocole PWA standard.
...
The application MUST fonctionner entièrement offline après la première installation, sans nécessiter de connexion internet.
...
The application MUST utiliser Workbox pour gérer le Service Worker avec des stratégies de cache optimales et un système de mise à jour automatique.
...
The application MUST détecter automatiquement les nouvelles versions et proposer à l'utilisateur de mettre à jour sans perte de données.
...
The application MUST avoir des icônes de qualité pour toutes les plateformes et des splash screens pour une expérience d'installation professionnelle.
...
The application MUST détecter l'état online/offline et adapter l'UI en conséquence pour informer l'utilisateur.
...
The application MUST obtenir un score Lighthouse PWA supérieur à 90 pour garantir les meilleures pratiques PWA.
