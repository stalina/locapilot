...
The database MUST avoir un schéma complet couvrant toutes les entités métier de Locapilot avec types TypeScript et indexes optimisés.
...
The application MUST fournir des opérations CRUD (Create, Read, Update, Delete) typées et testées pour toutes les tables de la base de données.
...
The database MUST supporter les migrations pour faire évoluer le schéma sans perte de données lors des mises à jour de l'application.
...
The application MUST fournir des composables Vue réutilisables pour faciliter l'accès à la base de données dans les composants.
...
The user MUST pouvoir exporter toutes ses données en JSON et les réimporter pour backup ou migration.
...
The system MUST gérer correctement les relations entre entités (property ↔ lease, lease ↔ rent, etc.) avec helpers pour queries relationnelles.
